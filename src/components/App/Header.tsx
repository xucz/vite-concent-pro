...
import { Avatar, Layout, Popover, Switch, Tag } from 'antd';
  ...
  return (
    <div style={{padding: '12px 28px'}}>
      ...
      <div>
        ...
        <Blank/>
        <Switch checkedChildren="关闭暗黑边栏" unCheckedChildren="开启暗黑边栏" checked={siderThemeChecked}
                onChange={onSiderThemeChange}/>
      </div>
      <VerticalBlank/>
      <div>
        <Tag color="geekblue">顶栏设置：</Tag>
        <Blank/>
        <Switch checkedChildren="关闭暗黑顶栏" unCheckedChildren="开启暗黑顶栏" checked={headerThemeChecked}
                onChange={onHeaderThemeChange}/>
      </div>
    </div>
  );
}
...
function AppHeader() {
  ...
  const uiContent = <SettingPanel color={st.themeColor} onWebsiteColorChange={se.onWebsiteColorChange}
                                  ...
                                  siderThemeChecked={gcu.siderThemeSwitchChecked}
                                  headerThemeChecked={gcu.headerThemeSwitchChecked}
                                  onSiderThemeChange={se.onSiderThemeChange}
                                  onHeaderThemeChange={se.onHeaderThemeChange}/>;
